...
try {
  ...
  node scripts/snapshot.mjs 2>&1 | Tee-Object -FilePath $log -Append | Out-Null
  if ($LASTEXITCODE -ne 0) { Log "snapshot ล้มเหลว (exit $LASTEXITCODE) — ยกเลิก ไม่ push"; exit 1 }

  Log "=== เริ่มตรวจ sync audit ==="
  node scripts/sync-audit.mjs 2>&1 | Tee-Object -FilePath $log -Append | Out-Null
  ...
  git commit -m "data: รีเฟรช HDC snapshot และ sync audit ($(Get-Date -Format 'yyyy-MM-dd HH:mm'))" 2>&1 | Tee-Object -FilePath $log -Append | Out-Null
  git push origin HEAD 2>&1 | Tee-Object -FilePath $log -Append | Out-Null
